...
class Features
{
    ...
    {
        return in_array($feature, config('shieldify.features', []));
    }
    ...
    public static function optionEnabled(string $feature, string $option)
    {
        return static::enabled($feature) &&
               config("shieldify-options.{$feature}.{$option}") === true;
    }
    ...
    {
        if (!empty($options)) {
            config(['shieldify-options.two-factor-authentication' => $options]);
        }
